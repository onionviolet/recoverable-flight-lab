...
export function initModel(host,onSelect){
 ...
 return {getView(){return {position:camera.position.toArray(),up:camera.up.toArray(),zoom:camera.zoom,target:controls.target.toArray()};},restoreView(v){if(!v)return;camera.position.fromArray(v.position);camera.up.fromArray(v.up);camera.zoom=v.zoom;controls.target.fromArray(v.target);camera.updateProjectionMatrix();controls.update();render();},setMode(v){controls.enabled=v;render();},setPose(p){vehicle.stowed.visible=p==='stowed';vehicle.deployed.visible=p!=='stowed';render();},cutaway(v){vehicle.shells.forEach(m=>m.visible=!v);render();},explode(v){vehicle.nose.position.y=v?.65:0;vehicle.aft.position.y=v?-.65:0;render();},preset(p){const pos={home:[8,5,14],front:[0,0,17],side:[17,0,0],top:[0,17,.01]};camera.zoom=1;camera.updateProjectionMatrix();camera.position.set(...(pos[p]||pos.home));controls.target.set(0,0,0);camera.lookAt(0,0,0);controls.update();render();},zoom(d){camera.zoom=Math.max(.5,Math.min(2,camera.zoom/d));camera.updateProjectionMatrix();controls.update();render();},select(id){selected=id;highlight();},dispose(){ro.disconnect();controls.dispose();renderer.dispose();}};
}
